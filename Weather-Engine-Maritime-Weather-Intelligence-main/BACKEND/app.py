...
def forecast10():
    """
    Returns 10 days forecast using only free OpenWeather endpoints:
      - Current weather (/weather) for Day 1
      - 5-day / 3h forecast (/forecast), aggregated into daily
      - Extend with extrapolated days if still <10
    ...
    try:
        ...
        cw_params = {"appid": OPENWEATHER_API_KEY, "units": "metric"}
        if city:
            cw_params["q"] = city
        else:
            cw_params["lat"] = lat
        ...
        cw_r = requests.get(OPENWEATHER_CURRENT, params=cw_params, timeout=20)
        cw_r.raise_for_status()
        cw = cw_r.json()
        w = cw.get("weather", [{"main": "Clear", "description":"clear sky", "icon":"01d"}])[0]
        days.append({
            "day": "Today",
            "date": datetime.utcfromtimestamp(cw["dt"]).date().isoformat(),
            "wind": round(cw.get("wind", {}).get("speed", 4.0) * 1.94384),
            "waves": round((cw.get("wind", {}).get("speed", 4.0) * 0.3 + 0.8), 1),
            "temp": round(cw.get("main", {}).get("temp", 22)),
            "condition": w.get("main", "Clear"),
            "description": w.get("description", "clear sky"),
            "humidity": cw.get("main", {}).get("humidity", 60),
            "pressure": cw.get("main", {}).get("pressure", 1013),
            "visibility": str(cw.get("visibility", 10000) / 1000.0),
            "clouds": cw.get("clouds", {}).get("all", 20),
            "icon": w.get("icon", "01d"),
            "windDirection": cw.get("wind", {}).get("deg", 0),
            "source": "current"
        })

        # 2) Forecast (5d/3h) -> aggregate to daily
        ...
        # 3) If still <10 days, extrapolate
        while len(days) < 10 and len(days) > 0:
            ...
            new_day = {
                **prev,
                ...
                "temp": max(-10, min(45, prev["temp"] + 0)),
